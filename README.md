...
### Analysis (Project 2)
To analyze the situation we did a VADER analysis using twitter and the three chosen Cryptocurrencies. To startoff we created two empty dictionaries named hashtags_sent and tweet. These dictionaries consisted of compound, positive, neatral, negative, and sentimental hashtags or tweets. Using these dictionaries we concated the data with our cryptocurrencies. By doing this we able to categorize the effects of a tweet. Furthermore we encorporated these findings into graphs to visualize our findings.

### Findings and Next Steps (Project 2)
We ran out of time when it came to checking the correlation that tweets might have on closing prices of the three chosen cryptos; but based off of the table and graphs, we can say that for the most part there is no corelation. But we did realize, at times when there where certain spikes or drops in closing prices, there were was a corelation of positive tweets affecting spikes in prices, and negative tweets affecting drops in prices. So doing a straight up sentimental analysis to predict prices, would not help our customers. So what is next?
It did not make sense to display our sentiment analysis on our streamlit dashboard, as for the most part that would have not helped our users and would cause us to lose ratings and customers. If we still choose do a sentiment analysis, something more interesting to do would be to rank the impact that certain figures might have on tweet related to cryptos. We have seen in the past that important figures, like Elon Musk or government heads, have a import say in the economy and their tweeets have shifted cryptos. So if we can rank the importance that certain figure's tweets have on cryptos, we can analyze to see the correlation that certain figures' tweets have on cryptos. Our prediction is that important figures, like Elon Musk, will have a better corellation than some random individual tweeting something.
When we are confident enough to assist users with a tool to predict the future of cryptos, we will then encorporate it our streamlit dashboard to assistr users.
