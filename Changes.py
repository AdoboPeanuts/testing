...
if asset_categories == "Cryptocurrencies":
   ...
   # Percentage return global code  
        ...
        #Defining the close and pct change
        crypto_df_close = crypto_df_new.drop(columns= ['open','high','low'])
        pct_change = crypto_df_close.pct_change()
        
        # Rename percentage change columns 
        pct_change.rename(columns= {'close':'pct_daily_return'})
        pct_change = pct_change.dropna(inplace=True)  
