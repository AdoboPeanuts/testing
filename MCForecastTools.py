...
class MCSimulation:
    ...
    def calc_cumulative_return(self):
        """
        Calculates the cumulative return of a cryptocurrency over time using a Monte Carlo simulation (Brownian motion with drift).

        """
        
        # Get closing prices of each cryptocurrency
        last_prices = self.portfolio_data.xs('close',level=1,axis=1)[-1:].values.tolist()[0]
        
        # Calculate the mean and standard deviation of daily returns for each cryptocurrency
        ...
        # Run the simulation of projecting cryptocurrency prices 'nSim' number of times
        for n in range(self.nSim):
        
            if n % 10 == 0:
                print(f"Running Monte Carlo simulation number {n}.")
        
            # Create a list of lists to contain the simulated values for each cryptocurrency
            simvals = [[p] for p in last_prices]
    
            # For each cryptocurrency in our data:
    ...
    def plot_simulation(self):
        """
        Visualizes the simulated cryptocurrency trajectories using calc_cumulative_return method.
    ...
    def summarize_cumulative_return(self):
        """
        Calculate final summary statistics for Monte Carlo simulated cryptocurrency data.
